...
| Watch baseline | `<state>/watch-baseline.json`, 0600 |
...
- `watch-baseline.json` — derived; maintained by `watch`, regenerated by `watch --reset`
- `custom_literals.json` — CLI-managed registry; mutated only through the `literals` commands
...
against your own `.env` files**: if a value currently in a secret-eligible
`.env` key appears anywhere in the scan, it is reported as a **KNOWN**
(identity-verified) row, marked `KNOWN (your .env values) — N in M files`.
...
```text
info-guard build [ENV_FILE ...]                  Build redact_patterns.json from .env sources
info-guard status                                Summary of the current pattern file
info-guard preflight [DIR ...] [--full] [--json] [--json-out FILE]
info-guard watch [DIR ...] [--reset] [--json] [--json-out FILE]
info-guard setup [--all]
...
info-guard literals add --from SOURCE:KEY
info-guard literals add --kind honeytoken                  plant a canary (ht- + 24 hex)
info-guard literals list [--json]
info-guard literals remove ID
info-guard literals rotate VALUE_ID [--json]      replacement via stdin only
info-guard rotate-candidates [--json]
info-guard discover [PATH...] [--json]
info-guard view systemd-unit|docker-env|compose-config|file <target>
info-guard env [FILE] [--check|--keys]            keys + lengths only, never values
info-guard pipe                                    mask any stdin stream
```
...
24 CSPRNG hex; the full value prints once to stderr — plant it somewhere
a scanner would find it, never in the scan dirs). Any exact match in a
scan is a **canary-touch** — a HONEYTOKEN finding, exit 4, sticky until
